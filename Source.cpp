...
void applyBinaryOperation(const operation& op, const long long &a, const long long &b, long long &c) {
	switch (op) {
	case (Sum): {
		c = sum(a, b);
		break;
...
void applyBinaryOperation(const operation &op, const long double &a, const long double &b, long double &c) {
	switch (op) {
	case (Sum): {
		c = sum(a, b);
		break;
